...
class Zombie:
    ...
    def run_from_boy(self):
        self.state = 'Walk'
        self.run_away_from()
        if self.ball_more_than_boy():
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING

    def ball_more_than_boy(self):
    ...
    def build_behavior_tree(self):
        ...
        a4 = Action('순찰 위치 가져오기', self.get_patrol_location)

        a5 = Action('소년에게로 접근', self.move_to_boy)

        c2 = Condition('좀비의 공 수가 소년보다 적은가?',self.ball_more_than_boy)
        ...
        root = run_boy = Sequence('좀비가 도망', c2, a6)
        root = chase = Sequence('추적', c1, a4)
        root = patrol = Sequence('순찰', a5, a2)
        root = chase_boy_or_run_boy = Selector('좀비가 추적 또는 좀비가 도망' ,run_boy, a5)
        root = close_to_boy = Sequence('소년이 근처에 있을때', c1, chase_boy_or_run_boy )
        root = chase_or_flee = Selector('추적 또는 배회', close_to_boy, wander)
